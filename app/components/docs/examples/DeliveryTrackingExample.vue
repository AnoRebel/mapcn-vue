...
const routePath = computed(() =>
  stops.map((s) => s.position as [number, number]),
);

// Animation state
const isPlaying = ref(false);
const currentStopIndex = ref(0);
const vehiclePosition = ref<[number, number]>(
  stops[0]!.position as [number, number],
);
...
<template>
  <div class="space-y-4">
      ...
      <CardContent class="p-4 space-y-4">
        ...
        <div class="text-sm">
          ...
          <span class="font-medium">{{
            stops[currentStopIndex]?.name || "Complete"
          }}</span>
        </div>
        ...
        <div class="flex gap-2">
          ...
          <Button variant="outline" size="sm" @click="resetAnimation">
    ...
    <div class="h-[400px] w-full rounded-lg overflow-hidden border">
      <Map :center="vehiclePosition" :zoom="13" class="h-full">
        <MapRoute
        ...
        >
          <MarkerContent>
            <div
              class="w-8 h-8 rounded-full bg-blue-500 border-2 border-white shadow-lg flex items-center justify-center animate-pulse"
            >
    ...
    <p class="text-sm text-muted-foreground">
      Delivery tracking simulation showing vehicle movement along a route with
      multiple stops. Progress tracking, ETA updates, and real-time position
      visualization.
    </p>
  </div>
</template>

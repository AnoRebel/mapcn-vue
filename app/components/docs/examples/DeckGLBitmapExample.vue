...
const layers = computed(() => [
  new BitmapLayer({
    id: 'bitmap-layer',
    bounds: [imageBounds.left, imageBounds.bottom, imageBounds.right, imageBounds.top],
...
<template>
  <div class="space-y-4">
    ...
    <div class="h-[400px] w-full rounded-lg overflow-hidden border">
      <Map :center="[-74.00, 40.715]" :zoom="13" class="h-full">
        <DeckGLOverlay :layers="layers" :interleaved="true" />
      </Map>
    </div>

    <p class="text-sm text-muted-foreground">
      BitmapLayer displays georeferenced images on the map. Provide the image URL 
      and bounds (left, bottom, right, top) to position the image correctly.

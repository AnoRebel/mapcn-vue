...
const unemploymentByState: Record<string, number> = {
  Alabama: 2.8, Alaska: 5.3, Arizona: 3.7, Arkansas: 3.5, California: 5.3,
  Colorado: 3.4, Connecticut: 4.2, Delaware: 4.4, Florida: 3.3, Georgia: 3.4,
  Hawaii: 3.0, Idaho: 3.1, Illinois: 4.8, Indiana: 3.4, Iowa: 2.9,
  Kansas: 2.9, Kentucky: 4.3, Louisiana: 4.0, Maine: 3.0, Maryland: 2.9,
  Massachusetts: 3.5, Michigan: 4.1, Minnesota: 3.2, Mississippi: 3.8, Missouri: 3.4,
  Montana: 3.1, Nebraska: 2.9, Nevada: 5.1, 'New Hampshire': 2.7, 'New Jersey': 4.2,
  'New Mexico': 4.0, 'New York': 4.5, 'North Carolina': 3.4, 'North Dakota': 2.5, Ohio: 4.0,
  Oklahoma: 3.4, Oregon: 4.3, Pennsylvania: 3.7, 'Rhode Island': 4.1, 'South Carolina': 3.3,
  'South Dakota': 2.7, Tennessee: 3.3, Texas: 4.0, Utah: 3.0, Vermont: 2.6,
  Virginia: 3.1, Washington: 4.2, 'West Virginia': 4.1, Wisconsin: 3.1, Wyoming: 3.4,
}
...
const legendItems = colorThresholds.map((threshold, index) => ({
  ...
  label: index === 0 ? `< ${threshold.value}%` : 
         index === colorThresholds.length - 1 ? `> ${colorThresholds[index - 1].value}%` :
    ...
    <Card v-if="selectedStateData">
      <CardContent class="p-4">
        <div class="flex items-center justify-between">
          ...
          <div class="text-2xl font-bold" :style="{ color: getColor(selectedStateData.rate) }">

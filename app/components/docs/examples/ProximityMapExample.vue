...
const facilities: FeatureCollection = {
  ...
  features: [
    {
      type: "Feature",
      properties: { name: "Hospital A", type: "hospital" },
      geometry: { type: "Point", coordinates: [-74.006, 40.7128] },
    },
    {
      type: "Feature",
      properties: { name: "Hospital B", type: "hospital" },
      geometry: { type: "Point", coordinates: [-73.985, 40.748] },
    },
    {
      type: "Feature",
      properties: { name: "School A", type: "school" },
      geometry: { type: "Point", coordinates: [-73.975, 40.758] },
    },
    {
      type: "Feature",
      properties: { name: "School B", type: "school" },
      geometry: { type: "Point", coordinates: [-73.995, 40.735] },
    },
    {
      type: "Feature",
      properties: { name: "Park A", type: "park" },
      geometry: { type: "Point", coordinates: [-73.965, 40.768] },
    },
    {
      type: "Feature",
      properties: { name: "Park B", type: "park" },
    ...
    <Card>
      <CardContent class="p-4 space-y-4">
        ...
        <div v-if="showProximity" class="space-y-2">
          ...
          <input
            v-model.number="proximityRadius"
            type="range"
            min="0.005"
            max="0.02"
            step="0.001"
          ...
          <span class="text-xs text-muted-foreground"
            >{{ (proximityRadius * 111).toFixed(1) }} km radius</span
          >
        </div>
        ...
        <div class="space-y-1 pt-2 border-t">
          ...
          <div
            v-for="(info, type) in facilityTypes"
            :key="type"
                ...
                features: facilities.features.map((f) => {
                  ...
                  return {
                    ...
                    geometry: {
                      ...
                      coordinates: [
                        [
                          [lng - proximityRadius * (parseInt(time) / 5), lat],
                          [lng, lat + proximityRadius * (parseInt(time) / 5)],
                          [lng + proximityRadius * (parseInt(time) / 5), lat],
                          [lng, lat - proximityRadius * (parseInt(time) / 5)],
                          [lng - proximityRadius * (parseInt(time) / 5), lat],
                        ],
          ...
          :style="
            {
              ...
              circleColor: [
                ...
                ['get', 'type'],
                'hospital',
                facilityTypes.hospital.color,
                'school',
                facilityTypes.school.color,
                'park',
                facilityTypes.park.color,
                '#999',
              ],
              circleOpacity: 0.9,
              strokeColor: '#fff',

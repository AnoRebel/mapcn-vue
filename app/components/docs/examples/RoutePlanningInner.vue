...
import { MapRoute, MapMarker, MarkerContent, useMap } from "~~/registry/map";
...
<template>
  <div
    class="absolute top-4 left-4 z-10 bg-background/90 backdrop-blur rounded-lg shadow-lg border p-3 space-y-2 min-w-[200px]"
  >
    <div class="space-y-2">
      <button
      ...
      >
        <div class="flex-1">
          <div class="flex items-center gap-2">
            ...
            <span
              v-if="route.isFastest"
              class="px-1.5 py-0.5 text-[10px] bg-green-500/20 text-green-600 rounded font-medium"
            >
              Fastest
            </span>
          </div>
          <div
            class="flex items-center gap-2 text-sm text-muted-foreground mt-0.5"
          >
            <Route class="size-4" />
  ...
  <MapMarker :longitude="4.9" :latitude="52.37">
    <MarkerContent>
      <div
        class="size-5 rounded-full bg-green-500 border-2 border-white shadow-lg"
      />
    </MarkerContent>
  </MapMarker>
  ...
  <MapMarker :longitude="4.48" :latitude="51.92">
    <MarkerContent>
      <div
        class="size-5 rounded-full bg-red-500 border-2 border-white shadow-lg"
      />
    </MarkerContent>
  </MapMarker>
</template>

...
const citiesData: FeatureCollection = {
  ...
  features: [
    {
      type: "Feature",
      properties: { name: "New York", population: 8400000, category: "mega" },
    ...
    {
      type: "Feature",
      properties: { name: "Chicago", population: 2700000, category: "large" },
      geometry: { type: "Point", coordinates: [-87.6298, 41.8781] },
    },
    {
      type: "Feature",
      properties: { name: "Houston", population: 2300000, category: "large" },
      geometry: { type: "Point", coordinates: [-95.3698, 29.7604] },
    },
    {
      type: "Feature",
      properties: { name: "Phoenix", population: 1600000, category: "medium" },
    ...
    {
      type: "Feature",
      properties: { name: "Dallas", population: 1300000, category: "medium" },
      geometry: { type: "Point", coordinates: [-96.797, 32.7767] },
    },
    {
      type: "Feature",
      properties: { name: "San Jose", population: 1000000, category: "small" },
      geometry: { type: "Point", coordinates: [-121.8947, 37.3382] },
    },
  ],
};
...
const categories = {
  ...
  large: {
    label: "Large Cities (1.5M-3M)",
    color: "#ea580c",
    visible: ref(true),
  },
  medium: {
    label: "Medium Cities (1M-1.5M)",
    color: "#f59e0b",
    visible: ref(true),
  },
...
const selectedCityData = computed(() => {
  ...
  return citiesData.features.find(
    (f) => f.properties?.name === selectedCity.value,
  );
});
</script>
...
<template>
  <div class="space-y-4">
        ...
        <div class="flex items-center justify-between">
          <div>
            <h3 class="font-semibold">
              {{ selectedCityData.properties?.name }}
            </h3>
            <p class="text-sm text-muted-foreground">
              Population:
              {{ selectedCityData.properties?.population?.toLocaleString() }}
            </p>
          </div>
          <button
            ...
            :style="
              {
                ...
                circleColor: [
                  ...
                  ['get', 'category'],
                  'mega',
                  categories.mega.color,
                  'large',
                  categories.large.color,
                  'medium',
                  categories.medium.color,
                  'small',
                  categories.small.color,
                  '#999',
                ],
                circleOpacity: 0.8,
                strokeColor: '#fff',
    ...
    <p class="text-sm text-muted-foreground">
      Interactive legend demonstrating category filtering. Click legend items to
      toggle visibility of different city categories. Click map markers to see
      city details.
    </p>
  </div>
</template>

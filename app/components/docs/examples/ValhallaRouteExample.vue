...
async function calculateRoute() {
  ...
  route.value = Array.from({ length: numPoints }, (_, i) => {
    ...
    return [
      start.value.lng +
        (end.value.lng - start.value.lng) * t +
        (Math.random() - 0.5) * 0.01,
      start.value.lat +
        (end.value.lat - start.value.lat) * t +
        (Math.random() - 0.5) * 0.01,
    ] as [number, number];
  });
  ...
  const speed =
    profile.value === "driving" ? 50 : profile.value === "cycling" ? 15 : 5;
  routeInfo.value = {
    distance: dist,
...
<template>
  <div class="space-y-4">
      ...
      <CardContent class="p-4 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-2">
            ...
            <div class="flex gap-2">
              <Input
                v-model.number="start.lng"
                placeholder="Lng"
                class="h-8 text-sm"
              />
              <Input
                v-model.number="start.lat"
                placeholder="Lat"
                class="h-8 text-sm"
              />
            </div>
          ...
          <div class="space-y-2">
            ...
            <div class="flex gap-2">
              <Input
                v-model.number="end.lng"
                placeholder="Lng"
                class="h-8 text-sm"
              />
              <Input
                v-model.number="end.lat"
                placeholder="Lat"
                class="h-8 text-sm"
              />
            </div>
    ...
    <p class="text-sm text-muted-foreground">
      Route planning example using Valhalla-style routing. In production,
      connect to
      <a

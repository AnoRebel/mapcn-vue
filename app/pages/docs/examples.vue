...
useSeoMeta({
  title: 'Examples',
  description: 'Comprehensive collection of interactive map examples with code.',
})

const examples = {
  globe: {
    title: '🌍 Globe Examples',
    description: '3D globe visualizations with satellite imagery',
    items: [
      { name: 'Earthquake Globe', component: 'EarthquakeGlobeExample', deps: ['deck.gl'], desc: '3D globe showing worldwide seismic activity with satellite tiles' },
    ],
  },
  valhalla: {
    ...
    items: [
      { name: 'Route Planning', component: 'ValhallaRouteExample', deps: [], desc: 'A-to-B routing with driving/cycling/walking profiles' },
      { name: 'Multi-Stop Routes', component: 'ValhallaMultiStopExample', deps: [], desc: 'Traveling salesman optimization with multiple stops' },
      { name: 'Delivery Tracking', component: 'DeliveryTrackingExample', deps: [], desc: 'Real-time delivery simulation with animated vehicle' },
    ],
  },
  maplibre: {
    title: '🗺️ MapLibre Essentials',
    ...
    items: [
      ...
      { name: 'Live Tracking', component: 'AnimatedRouteExample', deps: [], desc: 'Animated vehicle tracking with configurable speeds' },
    ],
  },
  layers: {
    title: '📊 Data Layers',
    ...
    items: [
      { name: 'GeoJSON Layers', component: 'GeoJsonLayersExample', deps: [], desc: 'Points, lines, and polygons from GeoJSON data' },
      { name: 'Choropleth Maps', component: 'ChoroplethExample', deps: [], desc: 'Color-coded regions with data-driven styling' },
      { name: 'Rich Popups', component: 'RichPopupsExample', deps: [], desc: 'Complex popups with images and actions' },
    ],
  },
  deckglCore: {
    title: '⚡ deck.gl Core',
    ...
    items: [
      { name: 'Scatterplot', component: 'DeckGLScatterplotExample', deps: ['deck.gl'], desc: 'High-performance point rendering with 1000s of points' },
      { name: 'Arc Layer', component: 'DeckGLArcExample', deps: ['deck.gl'], desc: 'Great circle arcs for flight routes' },
      { name: 'Line Layer', component: 'DeckGLLineExample', deps: ['deck.gl'], desc: 'Straight-line connections between points' },
  ...
  deckglAgg: {
    ...
    items: [
      { name: 'Heatmap', component: 'DeckGLHeatmapExample', deps: ['deck.gl'], desc: 'Density visualization for point clusters' },
      { name: 'Hexagon Layer', component: 'DeckGLHexagonExample', deps: ['deck.gl'], desc: '3D hexagonal binning for aggregated data' },
    ],
  },
  deckglGeo: {
    ...
    items: [
      { name: 'MVT Layer', component: 'DeckGLMVTExample', deps: ['deck.gl'], desc: 'Mapbox Vector Tiles rendering' },
      { name: 'Tile Layer', component: 'DeckGLTileExample', deps: ['deck.gl'], desc: 'Raster tile overlay' },
...
<template>
  ...
  >
    <DocsSection id="overview" title="Overview">
      <p>
        This collection includes {{ Object.values(examples).reduce((acc, cat) => acc + cat.items.length, 0) }}+ 
        interactive examples demonstrating various mapping capabilities. Each example is designed to be 
        copy-paste ready and follows our Vue 3 Composition API patterns.
      </p>
      <DocsNote>
        <strong>Note:</strong> Examples marked with <DocsCode>deck.gl</DocsCode> require additional dependencies. 
        See each section for installation commands.
      </DocsNote>
    </DocsSection>
    ...
    <DocsSection id="globe" :title="examples.globe.title">
      ...
      <div class="mt-4 space-y-2">
        <div v-for="item in examples.globe.items" :key="item.name" class="p-3 bg-muted rounded-lg">
          <div class="flex items-center justify-between">
            <span class="font-medium">{{ item.name }}</span>
            <span v-if="item.deps.length" class="text-xs bg-primary/10 text-primary px-2 py-1 rounded">
              {{ item.deps.join(', ') }}
            </span>
          </div>
    ...
    <DocsSection id="valhalla" :title="examples.valhalla.title">
      ...
      <div class="mt-4 space-y-2">
        <div v-for="item in examples.valhalla.items" :key="item.name" class="p-3 bg-muted rounded-lg">
          <div class="flex items-center justify-between">
            <span class="font-medium">{{ item.name }}</span>
            <span class="text-xs bg-green-500/10 text-green-600 px-2 py-1 rounded">No extra deps</span>
          </div>
          <p class="text-sm text-muted-foreground mt-1">{{ item.desc }}</p>
        </div>
      </div>
      <p class="mt-4 text-sm text-muted-foreground">
        Connect to <DocsLink href="https://valhalla.github.io/valhalla/" external>Valhalla</DocsLink> 
    ...
    <DocsSection id="maplibre" :title="examples.maplibre.title">
      ...
      <div class="mt-4 space-y-2">
        <div v-for="item in examples.maplibre.items" :key="item.name" class="p-3 bg-muted rounded-lg">
          <div class="flex items-center justify-between">
            <span class="font-medium">{{ item.name }}</span>
            <span class="text-xs bg-green-500/10 text-green-600 px-2 py-1 rounded">No extra deps</span>
          </div>
    ...
    <DocsSection id="layers" :title="examples.layers.title">
      ...
      <div class="mt-4 space-y-2">
        <div v-for="item in examples.layers.items" :key="item.name" class="p-3 bg-muted rounded-lg">
          <div class="flex items-center justify-between">
            <span class="font-medium">{{ item.name }}</span>
            <span class="text-xs bg-green-500/10 text-green-600 px-2 py-1 rounded">No extra deps</span>
          </div>
    ...
    <DocsSection id="deckgl-core" :title="examples.deckglCore.title">
      ...
      <pre class="mt-2 p-3 bg-muted rounded-lg text-sm"><code>{{ examples.deckglCore.deps }}</code></pre>
      <div class="mt-4 space-y-2">
        <div v-for="item in examples.deckglCore.items" :key="item.name" class="p-3 bg-muted rounded-lg border-l-4 border-primary">
          <div class="flex items-center justify-between">
            <span class="font-medium">{{ item.name }}</span>
    ...
    <DocsSection id="deckgl-aggregation" :title="examples.deckglAgg.title">
      ...
      <pre class="mt-2 p-3 bg-muted rounded-lg text-sm"><code>{{ examples.deckglAgg.deps }}</code></pre>
      <div class="mt-4 space-y-2">
        <div v-for="item in examples.deckglAgg.items" :key="item.name" class="p-3 bg-muted rounded-lg border-l-4 border-primary">
          <div class="flex items-center justify-between">
            <span class="font-medium">{{ item.name }}</span>
            <span class="text-xs bg-orange-500/10 text-orange-600 px-2 py-1 rounded">deck.gl</span>
          </div>
    ...
    <DocsSection id="deckgl-geographic" :title="examples.deckglGeo.title">
      ...
      <pre class="mt-2 p-3 bg-muted rounded-lg text-sm"><code>{{ examples.deckglGeo.deps }}</code></pre>
      <div class="mt-4 space-y-2">
        <div v-for="item in examples.deckglGeo.items" :key="item.name" class="p-3 bg-muted rounded-lg border-l-4 border-primary">
          <div class="flex items-center justify-between">
            <span class="font-medium">{{ item.name }}</span>
    ...
    <DocsSection title="Quick Start">
      ...
      <pre class="mt-2 p-4 bg-muted rounded-lg text-sm overflow-x-auto"><code>&lt;script setup lang="ts"&gt;
...
&lt;/template&gt;</code></pre>
      <p class="mt-4 text-muted-foreground">
        All examples are located in <DocsCode>app/components/docs/examples/</DocsCode> 

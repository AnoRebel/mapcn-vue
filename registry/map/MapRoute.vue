...
function animateRoute(coordinates: [number, number][]) {
  ...
  function step(currentTime: number) {
    ...
    const source = map.value!.getSource(sourceId.value) as MapLibreGL.GeoJSONSource | undefined
    if (source) {
      source.setData({
        type: 'Feature',
        properties: {},
...
watch(() => props.coordinates, (coordinates) => {
  if (!isLoaded.value || !map.value || coordinates.length < 2 || !isSetup.value) return
  const source = map.value.getSource(sourceId.value) as MapLibreGL.GeoJSONSource
  if (source) {
    if (props.animated) {

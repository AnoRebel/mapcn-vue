...
const props = withDefaults(defineProps<{
  ...
  pointColor?: string
  onPointClick?: (feature: GeoJSON.Feature<GeoJSON.Point>, coordinates: [number, number]) => void
...
watch(() => isLoaded.value, (loaded) => {
  ...
  handlers.handlePointClick = (e: MapMouseEvent & { features?: MapGeoJSONFeature[] }) => {
    ...
    props.onPointClick(feature as unknown as GeoJSON.Feature<GeoJSON.Point>, coordinates)
  }
  ...
  map.value.on('mouseenter', clusterLayerId.value, handlers.handleMouseEnterCluster)
  map.value.on('mouseleave', clusterLayerId.value, handlers.handleMouseLeaveCluster)
  map.value.on('mouseenter', unclusteredLayerId.value, handlers.handleMouseEnterPoint)
  map.value.on('mouseleave', unclusteredLayerId.value, handlers.handleMouseLeavePoint)
...
onUnmounted(() => {
  ...
  if (handlers.handleMouseEnterCluster) {
    map.value.off('mouseenter', clusterLayerId.value, handlers.handleMouseEnterCluster)
  }
  if (handlers.handleMouseLeaveCluster) {
    map.value.off('mouseleave', clusterLayerId.value, handlers.handleMouseLeaveCluster)
  }
  if (handlers.handleMouseEnterPoint) {
    map.value.off('mouseenter', unclusteredLayerId.value, handlers.handleMouseEnterPoint)
  }
  if (handlers.handleMouseLeavePoint) {
    map.value.off('mouseleave', unclusteredLayerId.value, handlers.handleMouseLeavePoint)
  }

  try {
    if (map.value.getLayer(clusterCountLayerId.value)) map.value.removeLayer(clusterCountLayerId.value)
    if (map.value.getLayer(unclusteredLayerId.value)) map.value.removeLayer(unclusteredLayerId.value)
    if (map.value.getLayer(clusterLayerId.value)) map.value.removeLayer(clusterLayerId.value)
